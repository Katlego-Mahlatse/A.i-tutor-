...
# Initialize ChromaDB (vector database for textbook search)
...
# Create or get collection for textbooks
...
class Answer(BaseModel):
    answer: str
    sources: List[dict]  # Page numbers and textbook references
    confidence: str
...
async def ask_question(question: Question):
    ...
    # Step 1: Anonymize student data (privacy layer)
    anonymous_question = question.question  # In production, remove any names/identifiers
    
    # Step 2: Search textbook database for relevant context
    results = textbook_collection.query(
        query_texts=[anonymous_question],
        n_results=3,  # Get top 3 most relevant chunks
        where={"subject": question.subject}  # Filter by subject
    )
...
async def get_subjects():
    """Get list of available subjects from uploaded textbooks"""
    # Query all unique subjects
...
async def health_check():
    """Check if Ollama and system are running"""
    try:
